...
class TableHeap {
 ...
 private:
  ...
  explicit TableHeap(BufferPoolManager *buffer_pool_manager, Schema *schema, Txn *txn, LogManager *log_manager,
    ...
    Page* rawPage = buffer_pool_manager->NewPage(first_page_id_);
    TablePage* page = reinterpret_cast<TablePage*>(rawPage);
    page->Init(first_page_id_, INVALID_PAGE_ID, log_manager, txn);
    bool unpinResult = buffer_pool_manager->UnpinPage(first_page_id_, true);
    if (!unpinResult) {
      std::cerr << "Failed to unpin page with id: " << first_page_id_ << std::endl;
    }
  };
